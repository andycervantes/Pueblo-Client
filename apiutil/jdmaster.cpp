...
class  my_decomp_master :public jpeg_decomp_master
{

	public :
		my_decomp_master()	{}
		virtual ~my_decomp_master() {}
		virtual void prepare_for_pass(j_decompress_ptr cinfo);
		virtual void finish_pass(j_decompress_ptr cinfo);
	...
	private :			
		...
		bool need_post_pass;	/* are we using full two-pass quantization? */
};

typedef class my_decomp_master * my_master_ptr;
...
void ChJPEG::jpeg_calc_output_dimensions (j_decompress_ptr cinfo)
...
void my_decomp_master::master_selection (j_decompress_ptr cinfo)
{
  long samplesperrow;
  JDIMENSION jd_samplesperrow;

  /* Initialize dimensions and other stuff */
  ChJPEG::jpeg_calc_output_dimensions(cinfo);
  ...
  if (cinfo->quantize_colors) {
	...
	total_passes++;
      ...
      ChJPEG::jinit_2pass_quantizer(cinfo);
#else
      ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
    } else {
#ifdef QUANT_1PASS_SUPPORTED
      ChJPEG::jinit_1pass_quantizer(cinfo);
#else
      ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
    }
...
#ifdef UPSAMPLE_MERGING_SUPPORTED
      ChJPEG::jinit_merged_upsampler(cinfo); /* does color conversion too */
#else
      ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
    } else {
      ChJPEG::jinit_color_deconverter(cinfo);
      ChJPEG::jinit_upsampler(cinfo);
    }
    ChJPEG::jinit_d_post_controller(cinfo, need_post_pass);
  }
  /* Inverse DCT */
  ChJPEG::jinit_inverse_dct(cinfo);
  ...
  if (cinfo->arith_code) {
    ...
    ChJPEG::jinit_huff_decoder(cinfo);

  ChJPEG::jinit_d_coef_controller(cinfo, (pass_type == preread_pass));
  ChJPEG::jinit_d_main_controller(cinfo, FALSE /* never need full buffer here */);
  /* Note that main controller is initialized even in raw-data mode. */

  /* We can now tell the memory manager to allocate virtual arrays. */
  ((j_common_ptr)cinfo)->mem->realize_virt_arrays((j_common_ptr) cinfo);
}
...
void my_decomp_master::prepare_for_pass (j_decompress_ptr cinfo)
{
  ...
  case main_pass:
    ...
    if (! cinfo->raw_data_out) 
    {
      ...
      if (cinfo->quantize_colors)
		cinfo->cquantize->start_pass(cinfo, need_post_pass);
      cinfo->post->start_pass(cinfo,
	    (need_post_pass ? JBUF_SAVE_AND_PASS : JBUF_PASS_THRU));
    }
  ...
  if (((j_common_ptr)cinfo)->progress != NULL) {
    ((j_common_ptr)cinfo)->progress->completed_passes = pass_number;
    ((j_common_ptr)cinfo)->progress->total_passes = total_passes;
  }
}
...
void my_decomp_master::finish_pass(j_decompress_ptr cinfo)
{
  ...
  case output_pass:
    ...
    pass_number++;
    pass_type = post_pass; /* in case need_post_pass is true */
    break;
#ifdef D_MULTISCAN_FILES_SUPPORTED
  case preread_pass:
    /* Count one pass done for each component in this scan */
    pass_number += cinfo->comps_in_scan;
...
void ChJPEG::jinit_master_decompress (j_decompress_ptr cinfo)
{
