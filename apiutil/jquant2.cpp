...
class  my_cquantizer2 : public jpeg_color_quantizer
{
	public :
		my_cquantizer2()	{}
		virtual ~my_cquantizer2() {}

		virtual void start_pass(j_decompress_ptr cinfo, bool is_pre_scan);
...
typedef class my_cquantizer2 * my_cquantize_ptr;
...
void my_cquantizer2::prescan_quantize (j_decompress_ptr cinfo, JSAMPARRAY input_buf,
		  JSAMPARRAY output_buf, int num_rows)
{
...
/* Master routine for color selection */
{
  ...
  boxlist = (boxptr) ((j_common_ptr) cinfo)->mem->alloc_small
...
void my_cquantizer2::pass2_no_dither (j_decompress_ptr cinfo,
...
void my_cquantizer2::pass2_fs_dither (j_decompress_ptr cinfo,
		 JSAMPARRAY input_buf, JSAMPARRAY output_buf, int num_rows)
...
/* Allocate and fill in the error_limiter table */
{
  ...
  table = (int *) ((j_common_ptr) cinfo)->mem->alloc_small
...
void my_cquantizer2::finish_pass1(j_decompress_ptr cinfo)
{
...
void my_cquantizer2::finish_pass2 (j_decompress_ptr cinfo)
{
  /* no work */
}
...
void my_cquantizer2::start_pass(j_decompress_ptr cinfo, bool is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i;
  ...
  if (is_pre_scan) {
    ...
    if (cinfo->dither_mode == JDITHER_FS)
      cquantize->color_quantize = pass2_fs_dither;
    else
      cquantize->color_quantize = pass2_no_dither;
...
void ChJPEG::jinit_2pass_quantizer (j_decompress_ptr cinfo)
{
  ...
  cquantize->histogram = (hist3d) ((j_common_ptr) cinfo)->mem->alloc_small
    ((j_common_ptr) cinfo, JPOOL_IMAGE, HIST_C0_ELEMS * SIZEOF(hist2d));
  for (i = 0; i < HIST_C0_ELEMS; i++) {
    cquantize->histogram[i] = (hist2d) ((j_common_ptr) cinfo)->mem->alloc_large
  ...
  if (cinfo->dither_mode == JDITHER_FS) {
    ...
    cquantize->fserrors = (FSERRPTR) ((j_common_ptr) cinfo)->mem->alloc_large
