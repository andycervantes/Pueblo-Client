...
class my_idct_controller : public jpeg_inverse_dct
{
	public :
		my_idct_controller()			{}
		virtual ~my_idct_controller()	{}	
		virtual void start_input_pass( j_decompress_ptr cinfo);
		virtual void start_output_pass (j_decompress_ptr cinfo);

	  /* Record the IDCT method type actually selected for each component */
	  J_DCT_METHOD real_method[MAX_COMPONENTS];
};

typedef class my_idct_controller * my_idct_ptr;
...
void my_idct_controller::start_input_pass (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (compptr->dct_table != NULL)
      continue;
    switch ( real_method[compptr->component_index]) {
...
void my_idct_controller::start_output_pass (j_decompress_ptr cinfo)
{
  jpeg_component_info *compptr;
  int ci;
...
void ChJPEG::jinit_inverse_dct (j_decompress_ptr cinfo)
{
  my_idct_ptr idct;
  int ci;
  jpeg_component_info *compptr;

  idct = new my_idct_controller;
  ASSERT( idct );
...
#ifdef IDCT_SCALING_SUPPORTED
    case 1:
      idct->inverse_DCT[ci] = jpeg_idct_1x1;
      idct->real_method[ci] = JDCT_ISLOW; /* jidctred uses islow-style table */
      break;
    case 2:
      idct->inverse_DCT[ci] = jpeg_idct_2x2;
      idct->real_method[ci] = JDCT_ISLOW; /* jidctred uses islow-style table */
      break;
    case 4:
      idct->inverse_DCT[ci] = jpeg_idct_4x4;
      idct->real_method[ci] = JDCT_ISLOW; /* jidctred uses islow-style table */
      break;
#endif
    case DCTSIZE:
...
#ifdef DCT_ISLOW_SUPPORTED
      case JDCT_ISLOW:
	idct->inverse_DCT[ci] = jpeg_idct_islow;
	idct->real_method[ci] = JDCT_ISLOW;
	break;
#endif
#ifdef DCT_IFAST_SUPPORTED
      case JDCT_IFAST:
	idct->inverse_DCT[ci] = jpeg_idct_ifast;
	idct->real_method[ci] = JDCT_IFAST;
	break;
#endif
#ifdef DCT_FLOAT_SUPPORTED
      case JDCT_FLOAT:
	idct->inverse_DCT[ci] = jpeg_idct_float;
