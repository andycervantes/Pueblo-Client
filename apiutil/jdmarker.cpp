...
#define INPUT_VARS(cinfo)  \
	class jpeg_source_mgr * datasrc = (cinfo)->src;  \
	const JOCTET * next_input_byte = datasrc->GetNextInputByte();  \
	size_t bytes_in_buffer = datasrc->GetBytesInBuffer()

/* Unload the local copies --- do this only at a restart boundary */
#define INPUT_SYNC(cinfo)  \
	( datasrc->SetNextInputByte( next_input_byte ),  \
	  datasrc->SetBytesInBuffer( bytes_in_buffer) )

/* Reload the local copies --- seldom used except in MAKE_BYTE_AVAIL */
#define INPUT_RELOAD(cinfo)  \
	( next_input_byte = datasrc->GetNextInputByte(),  \
	  bytes_in_buffer = datasrc->GetBytesInBuffer() )
...
/* Process a SOFn marker */
{
  ...
  if (cinfo->comp_info == NULL)	/* do only once, even if suspend */
    cinfo->comp_info = (jpeg_component_info *) ((j_common_ptr)cinfo)->mem->alloc_small
...
bool	jpeg_marker_reader::get_app0(j_decompress_ptr cinfo)
...
bool jpeg_marker_reader::get_app14(j_decompress_ptr cinfo)
...
bool jpeg_marker_reader::skip_variable (j_decompress_ptr cinfo)
...
int jpeg_marker_reader::read_markers (j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    ...
    case M_APP14:
    case M_APP15:
      if (! (*process_APPn[cinfo->unread_marker - (int) M_APP0]) (cinfo))
	return JPEG_SUSPENDED;
      break;
...
bool jpeg_marker_reader::read_restart_marker (j_decompress_ptr cinfo)
{
...
bool ChJPEG::jpeg_resync_to_restart (j_decompress_ptr cinfo)
{
...
void jpeg_marker_reader::reset_marker_reader (j_decompress_ptr cinfo)
{
  ...
  process_COM = skip_variable;
  for (int i = 0; i < 16; i++)
    process_APPn[i] = skip_variable;
  
  process_APPn[0] = get_app0;
  process_APPn[14] = get_app14;
...
void ChJPEG::jinit_marker_reader (j_decompress_ptr cinfo)
{

  /* Create subobject in permanent pool */
  if (cinfo->marker == NULL) {	/* first time for this JPEG object? */
  	cinfo->marker = new jpeg_marker_reader;
	ASSERT( cinfo->marker );
