...
int ChUnzip::GetZipType( const char* pstrZipFile )
{
	 ...
	 {	// first check if it is GZIP
		chuint8 tmpBuf[2];
		if ( pFile->read( tmpBuf, 2 ).gcount() == 2 )
		{
			...
			/*---------------------------------------------------------------------------
			  ...
			  ---------------------------------------------------------------------------*/
			    ...
			    else 
			    {
			        ...
			        {
			            cur_zipfile_bufstart = pFile->seekg( ziplen-tail_len, ios::beg ).tellg();
			    ...
			    /*-----------------------------------------------------------------------
			      ...
			      -----------------------------------------------------------------------*/
			        ...
			        for (i = 1;  i <= numblks;  ++i) 
			        {
			            ...
			            pFile->seekg(cur_zipfile_bufstart, ios::beg );
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    else 
    {
        if ((tail_len = ziplen % INBUFSIZ) > ECREC_SIZE) 
        {
            cur_zipfile_bufstart = m_pFile->seekg( ziplen-tail_len, ios::beg ).tellg();
    ...
    /*-----------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
        ...
        {
            cur_zipfile_bufstart -= INBUFSIZ;
            m_pFile->seekg(cur_zipfile_bufstart, ios::beg );
