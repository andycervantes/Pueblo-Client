...
#if 0
// made member functions of ChUnzip
...
int ChUnzip::extract_or_test_files()    /* return PK-type error code */
{
    uch *cd_inptr;
  ...
  ---------------------------------------------------------------------------*/
    ...
    while (members_remaining) {
        ...
        while (members_remaining && (j < DIR_BLKSIZ)) {
            ...
            if (m_flOptions & UNZIP_OPT_PROCESS_ALL_FILES ) {
                if (store_info())
			...
			#else
               const char **fnamev = fnv;   /* don't destroy permanent filename pointer */
                for (--fnamev; *++fnamev;)
    ...
    /*-----------------------------------------------------------------------
      ...
      -----------------------------------------------------------------------*/
        ...
        for (i = 0; i < j; ++i) {
            ...
            TRACE2( "debug: bufstart = %ld, cur_zipfile_bufstart = %ld\n",
            ...
            if (request < 0) {
                ...
                if (filnum == 0 && extra_bytes != 0L) {
                    ...
                    inbuf_offset = request % INBUFSIZ;
                    bufstart = request - inbuf_offset;
                    TRACE2( "debug: request = %ld, inbuf_offset = %ld\n",
                      request, inbuf_offset);
                    TRACE2( "debug: bufstart = %ld, cur_zipfile_bufstart = %ld\n",
                      bufstart, cur_zipfile_bufstart);
                } else {
            ...
            if (request < 0) {
                ...
                cur_zipfile_bufstart = m_pFile->seekg((LONGINT)bufstart, ios::beg ).tellg();
                
                if ((incnt = m_pFile->read((char *)inbuf,INBUFSIZ).gcount()) <= 0) {
			...
			// Added by PVS to extract to a different directory
			
			if (  m_flOptions & UNZIP_OPT_DST_RENAME )
			{
				lstrcpy( filename, m_pstrNewName );
			}


			if (  m_flOptions & UNZIP_OPT_DST_DIR  )
			{  // change the name, if there is volume name strip it
				string strDst;
				strDst = m_pstrDstPath;
				if ( strDst[strDst.GetLength() -1 ] != PATH_SEPARATOR )
				{
					strDst +=  PATH_SEPARATOR;
				}
				char * pstrSep = strchr( filename, '/' );
				if ( !pstrSep )
				{
					pstrSep = strchr( filename, '\\' );				
					if ( !pstrSep )
					{
						pstrSep = filename;
					}
             ...
             */
            //if (!tflag && !cflag) {
            if (!(m_flOptions & UNZIP_OPT_TEST) && !( m_flOptions & UNZIP_OPT_DISPLAY_FILE )) {
...
//startover:
				...
				if (  m_flOptions & (UNZIP_OPT_DST_DIR  | UNZIP_OPT_DST_RENAME ))
				{
					renamed = TRUE;
				}
...
//#endif
                    ...
                    /*  if (!error_in_archive)
                        ...
                        error_in_archive = PK_ERR;
                    TRACE2( "mapname(%s) returns error = %d\n", filename,
                      error);
                ...
                switch (check_for_newer(filename)) {
                    case DOES_NOT_EXIST:
                        ...
                        if ( (UNZIP_OPT_FRESHEN_ONLY & m_flOptions ) && !renamed)  /* don't skip if just renamed */
                            continue;   /* freshen (no new files):  skip */
                        break;
                    case EXISTS_AND_OLDER:
                        //if (overwrite_none)
                        if ( UNZIP_OPT_OVERWRITE_NONE & m_flOptions )
                            continue;   /* never overwrite:  skip file */
                        //if (!overwrite_all && !force_flag)
                        if (!(UNZIP_OPT_OVERWRITE_NONE & m_flOptions) 
                        			&& !( m_flOptions & UNZIP_OPT_FORCE ))
                            query = TRUE;
                        break;
                    case EXISTS_AND_NEWER:             /* (or equal) */
                        //if (overwrite_none || (uflag && !renamed))
                        if (UNZIP_OPT_OVERWRITE_NONE & m_flOptions || 
                        			(UNZIP_OPT_UPDATE & m_flOptions && !renamed))
                            continue;  /* skip if update/freshen & orig name */

                        //if (!overwrite_all && !force_flag)
                        if (!(UNZIP_OPT_OVERWRITE_ALL & m_flOptions)
                        			 && !( m_flOptions & UNZIP_OPT_FORCE ))
                            query = TRUE;
                        break;
                }
...
reprompt:
        ...
        cur_zipfile_bufstart = m_pFile->seekg((LONGINT)cd_bufstart, ios::beg ).tellg();
        m_pFile->read((char *)inbuf, INBUFSIZ);  /* were there b4 ==> no error */
        inptr = cd_inptr;
        incnt = cd_incnt;
        ++blknum;
...
void ChUnzip::makecrc( ulg* crc_32_tab )
/*
...
int ChUnzip::store_info()   /* return 0 if skipping, 1 if OK */
{
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
	...
	#else
    if ( m_flOptions & UNZIP_OPT_EBCIDIC_ASCII ) 
    {
       pInfo->textmode = pInfo->textfile;   /* auto-convert mode */
	}
	else
...
int ChUnzip::extract_or_test_member()    /* return PK-type error code */
{
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (m_flOptions & UNZIP_OPT_TEST ) {
        ...
        if ( m_flOptions & UNZIP_OPT_DISPLAY_FILE) {
            //outfile = stdout;
			TRACE( "Goes to the std out here" );
			ASSERT( false );
#ifdef DOS_NT_OS2
...
/*---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
    ...
    if (!(m_flOptions & UNZIP_OPT_TEST ) && !(m_flOptions & UNZIP_OPT_DISPLAY_FILE ))   /* don't close NULL file or stdout */
#endif
        close_outfile();
...
int ChUnzip::memextract(uch *tgt, ulg tgtsize, uch *src, ulg srcsize)   /* extract compressed extra */
    ...
    //ulg tgtsize, srcsize;                    /*  type error level */
{
    ...
    switch (method) {
        case STORED:
            ...
            outcnt = (int)csize;   /* for CRC calculation */
            break;
        case DEFLATED:
            if ((r = inflate()) != 0) {
                //FPRINTF(stderr, LoadFarStringSmall(ErrUnzipNoFile), r == 3?
