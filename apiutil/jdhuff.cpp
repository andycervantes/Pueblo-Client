...
class huff_entropy_decoder : public jpeg_entropy_decoder
{
	...
	private :
		bool fill_bit_buffer (working_state * state, int nbits);
		void fix_huff_tbl( j_decompress_ptr cinfo, JHUFF_TBL * htbl,
			     D_DERIVED_TBL ** pdtbl );
	int slow_DECODE (working_state * state, D_DERIVED_TBL * htbl, int min_bits);
	bool process_restart (j_decompress_ptr cinfo);

		savable_state saved;		/* Bit buffer & DC state at start of MCU */

		/* These fields are NOT loaded into local working state. */
		unsigned int restarts_to_go;	/* MCUs left in this restart interval */
		bool printed_eod;		/* flag to suppress extra end-of-data msgs */
...
typedef class huff_entropy_decoder * huff_entropy_ptr;
...
void huff_entropy_decoder::start_pass (j_decompress_ptr cinfo)
{
  int ci, dctbl, actbl;
  ...
  saved.bits_left = 0;
  saved.get_buffer = 0; /* unnecessary, but keeps Purify quiet */
  printed_eod = FALSE;
...
void huff_entropy_decoder::fix_huff_tbl (j_decompress_ptr cinfo, 
...
bool huff_entropy_decoder::fill_bit_buffer (working_state * state, int nbits)
/* Load up the bit buffer to a depth of at least nbits */
{
  ...
  while (bits_left < MIN_GET_BITS) {
	...
	return FALSE;
      next_input_byte = state->cinfo->src->GetNextInputByte();
      bytes_in_buffer = state->cinfo->src->GetBytesInBuffer();
    }
    bytes_in_buffer--;
...
int huff_entropy_decoder::slow_DECODE (working_state * state, D_DERIVED_TBL * htbl, int min_bits)
{
...
bool huff_entropy_decoder::process_restart (j_decompress_ptr cinfo)
{
  int ci;

  /* Throw away any unused bits remaining in bit buffer; */
  /* include any full bytes in next_marker's count of discarded bytes */
  cinfo->marker->discarded_bytes += saved.bits_left / 8;
  saved.bits_left = 0;
...
bool huff_entropy_decoder::decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  register int s, k, r;
  int blkn, ci;
  JBLOCKROW block;
  ...
  state.next_input_byte = cinfo->src->GetNextInputByte();
  state.bytes_in_buffer = cinfo->src->GetBytesInBuffer();
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    compptr = cinfo->cur_comp_info[ci];
    dctbl = dc_derived_tbls[compptr->dc_tbl_no];
    actbl = ac_derived_tbls[compptr->ac_tbl_no];
...
skip_ACs:
  ...
  cinfo->src->SetNextInputByte( state.next_input_byte );
  cinfo->src->SetBytesInBuffer( state.bytes_in_buffer );
...
void ChJPEG::jinit_huff_decoder (j_decompress_ptr cinfo)
{
  huff_entropy_ptr entropy;

  entropy = new huff_entropy_decoder;
  ASSERT( entropy );
