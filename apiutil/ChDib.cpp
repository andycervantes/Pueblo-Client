...
IMPLEMENT_SERIAL(ChDib, CObject, 0 /* Schema number */ )
...
ChDib::ChDib()
{
    ...
    m_pBits = NULL;
	m_pPalette = NULL;
	m_clrTransperent = 0xFFFFFFFF;
	m_flOptions = 0;

	m_pTransDib = 0;
	m_pMask = (void*)0xFFFFFFFF;
...
void ChDib::Serialize(CArchive& ar)
{
    ar.Flush();
    CFile* fp = ar.GetFile();
...
BOOL ChDib::Create(int iWidth, int iHeight, int iBitCount /*= 8 */ )
{
...
BOOL ChDib::Create(BITMAPINFO* pBMI, BYTE* pBits)
{
    ASSERT(pBMI);
    ASSERT(pBits);
    if (m_pBMI != NULL) FREE(m_pBMI);
...
BOOL ChDib::Load(LZHANDLE lzHdl)
{
    BOOL bIsPM = false;
    BITMAPINFO* pBmpInfo = NULL;
    BYTE* pBits = NULL;
...
BOOL ChDib::Load(const char* pszFileName, chuint32 flOption )
{
    CString strFile;    
...
BOOL ChDib::Load(WORD wResid, HINSTANCE hInstance)
{
    ASSERT(wResid);
...
BOOL ChDib::SetSize( long lWidth, long lHeight )
{
	...
	else
	{
		...
		{
		    pPal = ChDib::GetGrayScalePalette();
		}
		else
		{
		    pPal = ChDib::GetStdPalette();
		}
		if ( pPal )
		{
			hPalette = (HPALETTE)pPal->GetSafeHandle();
...
void ChDib::GetRect(CRect* pRect)
{
    pRect->top = 0;
    pRect->left = 0;
...
BOOL ChDib::Save(LPSTR pszFileName)
{
    CString strFile;    
