...
typedef ChSplay< ChModuleID, string> 				ChPuebloModuleIDMap;
typedef ChSplay< ChModuleID, ChModuleRunInfo*> 		ChModuleRunTable;
typedef ChSplay< string, ChRequestPending*> 		ChModulePendingList;
...
class ChModuleInfo
{
	public :
		...
		ChModuleInfo( ChModuleID idModule,  const string& strLibrary ) :
										m_idModule( idModule ),
		...
		const string&  	GetLibraryName() const		{ return m_strLibrary; }
...
typedef ChSplay< string, ChModuleInfo> 			ChPuebloModuleList;


class ChRequestPending
{
	public :
		ChRequestPending( const string& strModule );
		~ChRequestPending( );
...
class ChPuebloModuleManager
{
	public :
		...
		ChModuleID GetModuleID( const string& strModule );
		...
		bool OnLoadModule( const string& strModule, ChArgumentList *pArgList );

		const string&  GetAppDirectory() { return m_strAppDir; }
	...
	private :
		...
		void BuildModuleList();
 		...
 		bool FindModule( const string& strModuleName, string& strLibrary );
		LoadPuebloModule( const string& strModule, const string& strLibrary, ChArgumentList *pArgList );
