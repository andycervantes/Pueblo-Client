...
// $Header: /home/cvs/chaco/client/msw/ChMFrame.cpp,v 2.87 1996/07/02 19:56:22 pritham Exp $
		...
		hWndChild = ::GetNextWindow(hWndChild, GW_HWNDNEXT))
	{
		...
		if ( !boolChanged && ::GetTopWindow(hWndChild) != NULL)
		{
				...
				hWndChild = ::GetNextWindow(hWndChild, GW_HWNDNEXT))
			{
				...
				boolChanged =
					(bool)::SendMessage( hWndChild, WM_QUERYNEWPALETTE, 0, 0 );
			}
		}
	}
...
static void FormatTimeSpan( const ChTimeSpan& timeSpan, string& strTimeSpan )
{
	chint32	iTime;
	string	strTime;
...
void ChMainFrame::GetMessageString( UINT nID, string& rMessage ) const
{
	if (!GetPuebloCore()->GetMenuMgr()->GetMessageString( nID, rMessage ))
	{
		...
		#else
		{
			TRACE( "CFrameWnd::GetMessageString not available under Win16\n" );
			rMessage = TEXT("" );
		}
		#endif
	}
...
void ChMainFrame::OnHelpRegisterPueblo()
{
	if (GetPuebloCore()->DoRegistration() &&
			(GetPuebloCore()->GetClientMode() == ChClientCore::modeDisabled))
	{
		GetPuebloCore()->StartPueblo( string( "" ), ChClientCore::doLoginNotify );
	}
}
