...
// $Header: /home/cvs/chaco/client/msw/ChClCore.cpp,v 2.117 1996/09/15 19:32:33 pritham Exp $
...
CH_INTERN_FUNC( void )
PuebloWebPageLoadComplete( chint32 lError, const string& strURL,
...
void ChClientCore::StartPueblo( const string& strArg, chuint uOptions )
{
...
void ChClientCore::NewFrameWnd( const string& strArgs,
								const string& strLabel ) const
{
...
bool ChClientCore::ActivateFrame( const string& strFrameName )
{
...
void ChClientCore::SetFrameTitle( const string& strDocName )
{
	string				strAppName;
	string				strTitle;
...
string ChClientCore::GetFrameName() const
{
...
void ChClientCore::SetFrameName( const string& strFrameName )
{
...
string ChClientCore::GetModuleName( const ChModuleID& idModule )
{
...
------------------------------------------------------------------------------
	...
	Get the maininfo object for idModule
----------------------------------------------------------------------------*/
ChMainInfo* ChClientCore::GetMainInfo( const string& strModule,
									const char* pstrFrameName/* = 0 */ ) const
{
...
{    // Post a message to the core frame which will do the dispatch
	return m_pFrame->PostMessage( WM_CHACO_ASYNC_DISPATCH, idModule, (LPARAM)pMsg );
}
...
void ChClientCore::LoadClientModule( const string& strModuleName,
						...
						bool boolUseExisting /*= true*/ )
{
	...
	{
		ChLoadErrorMsg	msg( strModuleName, 0, string(""), 404,
								userData );
...
----------------------------------------------------------------------------*/
void ChClientCore::UnloadModule( const string& strModule ) const
{
...
void ChClientCore::DisplayStatus( const string& strStatus )
{
...
void ChClientCore::SetStatusPaneText( int iIndex, const string& strStatus )
{
	if ( iIndex < paneMax )
	{
...
----------------------------------------------------------------------------*/
bool ChClientCore::GetURL( const string strURL, chuint32 flOptions /*= 0 */, 
...
bool ChClientCore::PostURL( const string strURL, const char* pData, chint32 lLen, 
...
void ChClientCore::DisplayWebPage( const string& strURL, int iBrowser )
{
...
void ChClientCore::UpdateSessionPane( const string& strPaneText,
...
bool ChClientCore::DoRegistration()
{
	...
	{										/* User has registered but we were
		...
		if (ChUtil::FileExists( strRegInfoFile )) 
		{
			...
			fstream		streamIn( strRegInfoFile, ios::in, 0 );
	...
	{										/* Send the registration Info user
		...
		{  // save it just in case we cannot register with the server
			string strName;
			LOADSTRING( IDS_REGINFO_FILE, strName );
		  	string strRegInfoFile( GetModuleMgr()->GetAppDirectory() + strName );

			fstream streamOut( strRegInfoFile, ios::out, 0 );
