...
// $Header: /home/cvs/chaco/client/msw/ChClCore.h,v 2.44 1996/09/15 19:32:35 pritham Exp $
...
class ChClientCore : public ChCore
{
	...
	public:
		...
		virtual bool ActivateFrame( const string& strFrameName );

		virtual ChPersistentFrame* GetFrameWnd() const;
		virtual void SetFrameTitle(	const string& strDocName );
		virtual string GetFrameName() const;
		virtual void SetFrameName( const string& strFrameName );

		virtual void EnumerateFrames( ChFrameVisitor& enumFrame );


		virtual void DisplayStatus( const string& strStatus );
		virtual void SetStatusPaneText( int iIndex, const string& strStatus );

		virtual void UpdateSessionPane( const string& strPaneText, bool boolStartTracking = true );
		...
		virtual string GetModuleName( const ChModuleID& idModule );
		...
		virtual void UnloadModule( const string& strModule ) const;
		...
		virtual void DisplayWebPage( const string& strURL, int iBrowser );
	...
	protected:
		...
		void StartPueblo( const string& strArg, chuint uOptions = 0 );
		void StartShutdown();
