...
// $Header: /home/cvs/chaco/modules/client/msw/ChSound/ChSPrefs.h,v 1.18 1996/10/09 20:31:56 coyote Exp $
...
class ChAlertTime
{
	...
	public:
		...
		inline string GetAlertUnitsName() { return GetUnitsName(); }

		void SetAlertCount( chint32 lCount );
		void SetAlertUnits( string strUnits );
		void SetAlertUnits( AlertUnitType newUnits );

	protected:
		string GetUnitsName();
		AlertUnitType TranslateUnitsName( const string& strUnitsName );
...
class ChSoundPrefs : public ChPropertyBaseClass
{
	...
	public:
		...
		inline const string& GetAlertSoundPath() const
					{
	...
	protected:
		...
		string ExtractAlertSoundName();
		string ExtractAlertSoundTitle();
		string ExtractAlertSoundPath();
	...
	protected:
		...
		string				m_strAlertSoundPath;	// includes name
		string				m_strMidiFilePath;		// includes name
