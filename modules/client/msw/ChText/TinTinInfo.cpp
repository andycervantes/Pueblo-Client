...
// $Header: /home/cvs/chaco/modules/client/msw/ChText/TinTinInfo.cpp,v 2.1 1996/02/16 22:36:31 coyote Exp $
...
TinTinInfo::TinTinInfo( CWnd* pParent ) :
				ChDialog( (chparam)ChTextDLL.hModule, TinTinInfo::IDD,
							pParent ),
				m_iActions( 0 ),
				m_iAliases( 0 ),
				m_iAntiSubs( 0 ),
...
void TinTinInfo::SetFlags( bool boolActions, bool boolStartQuotes,
							bool boolSpeedWalking, bool boolSubs,
							bool boolPresubs )
{
	SetOnOff( m_strStartQuotes, boolStartQuotes );
	SetOnOff( m_strSpeedwalking, boolSpeedWalking );

	SetYesNo( m_strDoActions, boolActions );
...
void TinTinInfo::DoDataExchange( CDataExchange* pDX )
{
	...
	DDX_Text(pDX, IDC_STATIC_STARTQUOTES, m_strStartQuotes);
	DDX_Text(pDX, IDC_STATIC_PRESUBS, m_strPresubs);
	DDX_Text(pDX, IDC_STATIC_DO_SUBS, m_strDoSubs);
	DDX_Text(pDX, IDC_STATIC_DO_ACTIONS, m_strDoActions);
	//}}AFX_DATA_MAP
}
...
void TinTinInfo::SetYesNo( string& strField, bool boolYes )
{
	if (boolYes)
	{
		strField = "yes";
...
void TinTinInfo::SetOnOff( string& strField, bool boolOn )
{
	if (boolOn)
	{
		strField = "on";
...
void TinTinInfo::SetTrueFalse( string& strField, bool boolTrue )
{
	if (boolTrue)
	{
		strField = "true";
