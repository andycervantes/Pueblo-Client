...
// $Header: /home/cvs/chaco/modules/client/portable/ChGraphx/ChTrMonot.cpp,v 1.5 1996/09/14 22:28:44 pritham Exp $
...
int ChTriangulator::inside_polygon(trap_t *t)
{
  int rseg = t->rseg;
...
int ChTriangulator::new_chain_element()
{
  return ++chain_idx;
}


double ChTriangulator::get_angle(point_t *vp0, point_t *vpnext, point_t *vp1)  //static?
{
  point_t v0, v1;
...
int ChTriangulator::get_vertex_positions(int v0, int v1, int *ip, int *iq)
{
 ...
 */
int ChTriangulator::make_new_monotone_poly(int mcur, int v0, int v1)
{
  int p, q, ip, iq;
  int mnew = newmon();
  int i, j, nf0, nf1;
...
int ChTriangulator::monotonate_trapezoids(int n)
{
  register int i;
  int tr_start;

  memset((void *)vert, 0, m_segSize * sizeof(*vert));
  memset((void *)visited, 0, m_trSize * sizeof(*visited));
  memset((void *)mchain, 0, m_trSize * sizeof(*mchain));
  memset((void *)mon, 0, m_segSize * sizeof(*mon));
  
  /* First locate a trapezoid which lies inside the polygon */
  /* and which is triangular */
  for (i = 0; i < m_trSize; i++)
...
int ChTriangulator::traverse_polygon(int mcur, int trnum, int from, int dir)
{
  trap_t *t = &tr[trnum];
  int mnew;
  //int howsplit, tmp;
