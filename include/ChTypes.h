...
// $Header: /home/cvs/chaco/include/ChTypes.h,v 2.18 1996/01/02 23:58:07 pritham Exp $
...
class string : public String
{
  ...
  public:
	...
	string(const char* t) : String(t), buffer( 0 ) { };
	string(const char* t, int len) : String( t, len ), buffer( 0 ) { };
	string(const string& x) : String(x), buffer( 0 ) { };
	string(const SubString& x) : String(x), buffer( 0 ) { };
	string(char c) : String(c), buffer( 0 ) { };

	string& operator =  (const string& y) { String::operator=(y); return *this; };
	string& operator =  (const String& y) { String::operator=(y); return *this; };
	string& operator =  (const char* y) { String::operator=(y); return *this; };
	string& operator =  (char c) { String::operator=(c); return *this; };
	string& operator =  (const SubString& y) { String::operator=(y); return *this; };
	
	// New functions, defined for MSW CString compatibility:
	string Left(int len) { return at(0, len); };
	string Left(int len) const {
		string temp = *this;
		return temp.at(0, len); 
	};
	string Mid(int start, int end = -1) { 
		if (end == -1)
			return from( start );
		else
			return at( start, end );
...
//	int ReverseFind(const char* t) const { return rindex( chars(), t ); };
	string Right(int pos)  { return after(pos); };
	int CompareNoCase(const string& s) const { return fcompare( *this, s ); };
