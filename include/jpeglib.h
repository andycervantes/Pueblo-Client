...
#define jpeg_common_fields \
  class jpeg_error_mgr * err;	/* Error handler module */\
  class jpeg_memory_mgr * mem;	/* Memory manager module */\
  class jpeg_progress_mgr * progress; /* Progress monitor, or NULL if none */\
...
typedef class jpeg_common_struct * j_common_ptr;
//typedef class jpeg_compress_struct * j_compress_ptr;
typedef class jpeg_decompress_struct * j_decompress_ptr;
...
class  CH_EXPORT_CLASS jpeg_decompress_struct 
{
	public :
	...
	// cast operators to get at the common fields
	operator j_common_ptr() {	return &m_common; }

	//class jpeg_memory_mgr* mem;
	//class jpeg_error_mgr* err 
	//class jpeg_progress_mgr* progress;
   ...
   */
  class  jpeg_decomp_master * master;
  class  jpeg_d_main_controller * main;
  class  jpeg_d_coef_controller * coef;
  class  jpeg_d_post_controller * post;
  class  jpeg_marker_reader * marker;
  class  jpeg_entropy_decoder * entropy;
  class  jpeg_inverse_dct * idct;
  class  jpeg_upsampler * upsample;
  class  jpeg_color_deconverter * cconvert;
  class  jpeg_color_quantizer * cquantize;
};
...
{  
	public :
		...
		// virtual member functions for error handling
		virtual void init_error_mgr();
		virtual void error_exit( j_common_ptr cinfo );
		virtual void output_message( j_common_ptr cinfo );
		virtual void format_message( j_common_ptr cinfo, char * buffer );
		virtual void reset_error_mgr( j_common_ptr cinfo );
		virtual void emit_message (j_common_ptr cinfo, int msg_level);
...
class  jpeg_destination_mgr 
{
	public :
		...
		virtual void init_destination( j_compress_ptr cinfo );
		virtual void empty_output_buffer( j_compress_ptr cinfo );
		virtual void term_destination( j_compress_ptr cinfo );
...
class  jpeg_source_mgr 
{
	public :
		jpeg_source_mgr()	{}
		virtual ~jpeg_source_mgr()	{}

		virtual void init_source( j_decompress_ptr cinfo ) = 0;
		virtual bool fill_input_buffer( j_decompress_ptr cinfo ) = 0;
		virtual void skip_input_data( j_decompress_ptr cinfo, long num_bytes ) = 0;
		virtual bool resync_to_restart( j_decompress_ptr cinfo ) = 0;
		virtual void term_source( j_decompress_ptr cinfo ) = 0;
		// attributes
		const JOCTET * GetNextInputByte() { return next_input_byte; }
		size_t GetBytesInBuffer() { return bytes_in_buffer; }
		void SetNextInputByte( const JOCTET * pBuf) { next_input_byte = pBuf; }
		void SetBytesInBuffer( size_t iBytes) { bytes_in_buffer = iBytes; }
	private :
...
class jpeg_memory_mgr 
{
	...
	// override these to define your own memory manager
		...
		virtual jvirt_sarray_ptr request_virt_sarray( j_common_ptr cinfo,
							  ...
							  JDIMENSION samplesperrow,
							  JDIMENSION numrows,
							  JDIMENSION unitheight ) = 0;
		virtual jvirt_barray_ptr request_virt_barray(j_common_ptr cinfo,
							  int pool_id,
							  JDIMENSION blocksperrow,
							  JDIMENSION numrows,
							  JDIMENSION unitheight) = 0;
		virtual void realize_virt_arrays( j_common_ptr cinfo ) = 0;
		...
		virtual JBLOCKARRAY access_virt_barray( j_common_ptr cinfo,
						    ...
						    JDIMENSION start_row,
						    bool writable ) = 0;
		virtual void free_pool( j_common_ptr cinfo, int pool_id ) = 0;
		virtual void self_destruct(j_common_ptr cinfo) = 0;
 ...
 */
CH_TYPEDEF_FUNC( bool, jpeg_marker_parser_method )
...
CH_EXTERN_FUNC( JQUANT_TBL * )
jpeg_alloc_quant_table(j_common_ptr cinfo);

CH_EXTERN_FUNC( JHUFF_TBL * )
