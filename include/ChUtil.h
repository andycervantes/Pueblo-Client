...
// $Header: /home/cvs/chaco/include/ChUtil.h,v 2.31 1996/09/25 03:16:17 pritham Exp $
...
class ChTmpFileManager
{
	public :
		...
		void Add( const string& strFile ) 
			{  
				string *pNew = new string( strFile );
...
class CH_EXPORT_CLASS ChUtil
{
	public:
		...
		static bool Load( chparam resources, chint32 lKey, string& strData );
		static int	LoadString( UINT nID, string& strBuffer );

		static bool FileExists( const char* pstrFilename );
		static void GetAppDirectory( string& strDir );
		...
		static bool CreateDirectoryTree( const string& strPath ); 
		static void	AddFileToTempList( const string& strFile );
		static void	EmptyTempFileList();
		static void AddModuleToLoadList( const string& strModule );
		...
		static bool GetTempFileName( string& strTmpFile,
										const char* pstrTempPath = 0,
										const char* pstrPrefix = 0,
										const char* pstrExtn = 0 );

		static void HtmlAddNameValuePair( string& strData, const string& strName, const string& strValue );
		static void HtmlAddNameValuePair( string& strData, const string& strName, int iValue );


		#if defined( CH_UNIX )

		static const string& GetPuebloDir() { return m_strPuebloDirectory; }
		static void SetPuebloDir( const string& strPuebloDir )
				{
		...
		static void EncryptString( string& strSource, bool boolEncrypt,
									char cDelimiter = 0 );
		static void EncodeAttributeString( const string& strSource,
											string& strDest );
	...
	protected:
		...
		static void DoEncrypt( string& strSource, char cDelimiter );
		static void DoDecrypt( string& strSource );
		static void AppendChar( string& strDest, const char cAppend,
								bool boolForceHex = false );
		static char ReadChar( const char** ppstrSource );

		static bool IsOldEncryptString( string& strSource );
		static void OldEncryptString( string& strSource, bool boolEncrypt,
										char cDelimiter );

	protected:
		static string		m_strAppDirectory;
	...
	private :
		...
		static string		m_strPuebloDirectory;
