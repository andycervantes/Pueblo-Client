...
// $Header: /home/cvs/chaco/include/ChCore.h,v 2.36 1996/09/12 18:16:50 pritham Exp $
...
{
	public:
		virtual bool Visit( const string& strFrameName,
							ChCore* pCore ) = 0;
...
class CH_EXPORT_CLASS ChCore
{
	...
	public:
		...
		virtual void NewFrameWnd( const string& strArgs,
									const string& strLabel = "" ) const = 0;

		virtual bool ActivateFrame( const string& strFrameName ) = 0;

		virtual ChPersistentFrame* GetFrameWnd() const = 0;
		virtual void SetFrameTitle(	const string& strDocName ) = 0;
		virtual string GetFrameName() const = 0;
		virtual void SetFrameName( const string& strFrameName ) = 0; 
		...
		inline void Trace( const string& strText, chflag32 flType,
							bool boolHtml = false )
				{
		...
		virtual void DisplayStatus( const string& strStatus ) = 0;
		virtual void SetStatusPaneText( int iIndex, const string& strStatus ) = 0;
		virtual void UpdateSessionPane( const string& strPaneText, bool boolStartTracking = true ) = 0;
		...
		virtual string GetModuleName( const ChModuleID& idModule ) = 0;
		...
		virtual ChMainInfo* GetMainInfo( const string& strModule, 
										const char* pstrFrameName = 0 ) const = 0;

		virtual void UnloadModule( const ChModuleID& idModule ) const = 0;
		virtual void UnloadModule( const string& strModule ) const = 0;
		...
		virtual bool GetURL( const string strURL, chuint32 flOptions = 0, 
								ChHTTPStreamManager* pStream = 0,
								chparam userData = 0 ) = 0;
		virtual bool PostURL( const string strURL, const char* pData, chint32 lLen, 
								chuint32 flOptions = 0, 
								ChHTTPStreamManager* pStream = 0,
								chparam userData = 0 ) = 0;
		virtual void DisplayWebPage( const string& strURL, int iBrowser ) = 0;

		#if defined( CH_CLIENT )

		virtual void LoadClientModule( const string& strModuleName,
	...
	protected:
		void DoTrace( const string& strText, chflag32 flType, bool boolHtml );
