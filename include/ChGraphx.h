...
// $Header: /home/cvs/chaco/include/ChGraphx.h,v 2.17 1996/03/05 21:58:30 pritham Exp $
...
{
	public:
		ChSceneMsg( const string& strURL, const string& strFile, const string & strType, bool boolNew = true,  chparam data = 0 ) :
		...
		inline void GetParams( string& strURL, string& strFile, string & strType, bool& boolNew )
				{
					ChArchive	archive( this, modeRead );
					boolNew = (bool)GetParam1();
					archive >> strURL;
					archive >> strFile;
					archive >> strType;
				};
		inline void GetParams( string& strURL, string& strFile, string & strType, bool& boolNew, chparam &data )
				{
					ChArchive	archive( this, modeRead );
					boolNew = (bool)GetParam1();
					archive >> strURL;
...
class CH_EXPORT_CLASS ChCastMsg : public ChMsg
{
	...
	public:
	#if 0
		ChCastMsg( chuint32 luId, const string& strSpriteURL,
		...
		ChCastMsg( const string& strCastURL ) : ChMsg( CH_MSG_LOAD_CAST )
				{
					ChArchive	archive( this, modeWrite );
	...
	#if 0
		// direct only
		inline void GetParams( chuint32& luId, string& strSpriteURL, 
		...
		inline void GetParams(  string &strURL )	// indirect only
		{
...
class CH_EXPORT_CLASS ChAnchorMsg : public ChMsg
{
	...
	public:
		ChAnchorMsg( chuint32 id, ChRect& rcHot, string& strCmd, 
			string& strHint, chuint32 luCursor = CH_CURSOR_PICK ) :
		...
		ChAnchorMsg( chuint32 id, chuint32 luHotCastId, string& strCmd, 
			string& strHint, chuint32 luCursor = CH_CURSOR_PICK ) :
					ChMsg( CH_MSG_LOAD_ANCHOR )
				{
		...
		inline void GetParams( chuint32& id, ChRect& rcHot, string& strCmd, 
			string& strHint, chuint32& luCursor )
				{
					// should assert on type, or something nicer
					chuint32 luHotCastId = 0;
					ChArchive	archive( this, modeRead );
		...
		inline void GetParams( chuint32& id, chuint32& luHotCastId, string& strCmd, 
			string& strHint, chuint32& luCursor )
				{
					// should assert on type, or something nicer
					ChRect rcHot(0,0,0,0);
...
class CH_EXPORT_CLASS ChEnableDragMsg : public ChMsg
{
	public:
		...
		ChEnableDragMsg( const string& strURL ) :
