...
// $Header: /home/cvs/chaco/include/ChHTTP.h,v 2.49 1996/10/19 01:46:00 pritham Exp $
...
class CH_EXPORT_CLASS ChHTTPNotification
{
	public:
		...
		ChHTTPNotification( const string& strURL, const string& strFile, 
							const string& strMimeType, chparam userData ) :
		...
		ChHTTPNotification( int iError, const string& strErrMsg, const string& strURL,  chparam userData) :
		...
		ChHTTPNotification( int iType, const string& strMsg ) :
		...
		ChHTTPNotification( int iType, const string& strMsg, chuint32 luProgress, chuint32 luRange ) :
							m_iType(iType ), m_iError(0), 
							m_strURL( strMsg ),  m_userData( 0 ),
		...
		void  GetLoadCompleteInfo( string& strURL, string& strFile, 
							string& strMimeType, chparam& userData )
							{
								strURL = m_strURL;
		...
		void  GetLoadErrorInfo( int& iError, string& strMsg, string& strURL, chparam& userData )
							{
								strURL = m_strURL;
								strMsg = m_strData;
								iError = m_iError;
		...
		void  GetProgressInfo( string& strMsg, chuint32& luProgress, chuint32& luMax )
							{
								strMsg = m_strURL;
								luProgress = m_luProgress;
								luMax = m_luMaxRange;
							}
		void  GetAlertInfo( string& strMsg, chuint32& luType, chuint32& luButton )
							{
								strMsg = m_strURL;
								luType = m_luProgress;
								luButton = m_luMaxRange;
							}
		void  GetBeginInfo( string& strMsg )
							{
								strMsg = m_strURL;
							}
...
class CH_EXPORT_CLASS ChHTTPConn
{
	...
	public:
		...
		virtual void    GetUserAgent( string& strAgent )	{}
		virtual void    SetUserAgent( const string& strAgent )	{}
		...
		virtual bool GetURL( const string& strURL, 
		...
		virtual bool PostURL( const string& strURL, const char* pstrBody,
							...
							const char* pstrDefURL = 0, 
							chuint32 flOptions = 0, ChHTTPStreamManager* pStreamMgr = 0 ) = 0;
		virtual void QueryURL( const string& strFile, string& strURL, string& strMimeType ) = 0;

		virtual bool IsURLCached( const string& strURL, const char* pstrDefURL = 0 ) = 0;
		virtual bool IsVisitedURL( const string& strURL, const char* pstrDefURL = 0 ) = 0;
		...
		virtual bool DeleteCachedURL( const string& pstrURL, const char* pstrDefURL = 0 ) = 0;
		virtual void AbortRequests( bool boolAbortPrefetch = false, ChHTTPStreamManager* pStreamMgr = 0 ) = 0;

		virtual bool IsActive() = 0;
		virtual int  NumActive() = 0;
		virtual void GetProgressMsg( string& strMsg, int& iPercentComplete ) = 0; 
		...
		static int GetMimeType( const string& strType );
		static void GetMimeTypeByFileExtn( const char* pstrFile, string& strMimeType  );
		static void GetFileExtnByMimeType( int iMimeType, string& strExtn  );
		static void	AddMimeType( const string& strMime, const string& strFileExtn, 
							const string& strDesc );
 		static int GetFileOpenFilter( string& strFilter );
