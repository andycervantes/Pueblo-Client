...
// $Header: /home/cvs/chaco/include/ChRMenu.h,v 2.5 1996/03/07 21:13:23 pritham Exp $
...
class ChRMenu 
{
	protected:
		...
		string					m_strTitle;		// used only for destruction!
		ChRMenuMgr* 			m_pMenuMgr;		// Menu manager for this menu block

	public:
		
		ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChNativeMenu * pMenu, const ChMsgHandler pHandler );
		ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChNativeMenu * pMenuBar );
		ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChRMenuItem *pItem  );

		virtual ~ChRMenu();

		inline ChRMenuMgr* GetMenuMgr() const		{ return  m_pMenuMgr; }

		ChMenu* Install( ChMenu *pMenu, const string& strModuleName, bool boolToHead = true);
		...
		ChRMenuBlock* FindBlock(string strModName, chuint32 *pluBegin = 0);
...
class ChRMenuItem 
{
	...
	public:
		...
		bool IsChecked();
		string& GetTitle();
		inline string& GetDisplayTitle() { return m_strDisplayTitle; };
		...
		static string  SplitDisplayTitle(const string & strDisplayTitle );
...
class ChRMenuBlock 
{
	public:
		...
		ChRMenuBlock( string strModName, ChRMenu *pRMenu  );
		...
		string MakeSeparatorTitle( ChMenuItemId id ) const;
	...
	protected:
		...
		#if !defined( NO_TEMPLATES )
		ChSplay<string, ChRMenuItem *>	m_itemNames;	// RItems are keyed by itemname	  
		#else
		...
		class ChDeleteBlocksOp : public 
								#if !defined( NO_TEMPLATES )
								ChVisitor2<string, ChRMenuItem*>  
								#else 
								ChMenuItemSplayVisitor2
								#endif
		{
			public:
				bool Visit( const string& key, const pChRMenuItem& pBlock )
						{
							delete pBlock;
							return true;
						}
								...
								#endif
		{
		  ...
		  public:
			...
			bool Visit(const string& key, const pChRMenuItem& data){ 	m_luCount++;
																		return true;};
		};
								...
								#endif
		{
			public:
				#if !defined( NO_TEMPLATES )
				ChDeleteItemOp( ChMenu *pMenu, ChList<string> *pDeadList )
				...
				bool Visit( const string& key, const pChRMenuItem& pRMenu )
						{
							#if 0
...
class CH_EXPORT_CLASS ChRMenuMgr			// exactly one of these per client app
{
	protected:
		...
		#if !defined( NO_TEMPLATES )
		ChSplay<string, ChRMenu*>	m_menus;
	...
	//	static ChRMenuMgr*			m_pTheMgr;
		...
		class ChDeleteRMenuOp : public 
						#if !defined( NO_TEMPLATES )
						ChVisitor2<string, ChRMenu*>
						#else
						ChMenuMapSplayVisitor2
						#endif
		{
			public:
				bool Visit( const string& key, const pChRMenu& data )
						{
	...
	public:
		...
		bool Notify( ChMenuItemId id, chuint32 uMessage = CH_MSG_MENU_SELECT,
						bool* pboolEnabled = 0 );
		bool GetMessageString(UINT nID, string& rMessage);

		void RemoveRMenu(string& name ) { m_menus.Delete( name ); };
