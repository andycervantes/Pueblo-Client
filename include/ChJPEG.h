...
class CH_EXPORT_CLASS ChJPEG : public ChDib
{
    DECLARE_SERIAL(ChJPEG)
public:
    ChJPEG();
    virtual ~ChJPEG();
    virtual BOOL Load(const char* pszFileName = NULL, 
    				chuint32 flOption = ChDib::loadAuto );			// Load JPEG from disk file
...
private :
	...
	void jpeg_stdio_src (j_decompress_ptr cinfo, fstream * infile);
