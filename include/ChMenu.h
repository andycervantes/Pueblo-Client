...
// $Header: /home/cvs/chaco/include/ChMenu.h,v 2.6 1996/03/07 21:13:20 pritham Exp $
...
class CH_EXPORT_CLASS ChMenu				//  Should inherit from ChStreamable
{
	...
	public:
		...
		ChMenu( ChRMenuMgr* pMgr, string strTitle, const ChMsgHandler pDefHandler,
		...
		ChMenuItem* InsertItem( string strTitle, const ChMsgHandler pHandler = 0,
		...
		ChMenuItem* FindItem( string strTitle, chint16 sPosition = 0 );
		...
		ChMenu* Install( const string& strModuleName, bool boolToHead = true );
		void Uninstall();
...
{
	friend ChMenuItem* ChMenu::InsertItem( string strTitle, 
	...
	public:
		...
		inline string& GetHelpText() { return m_strHelp; }
		...
		ChMenuItem* SetTitle( string strTitle );
		ChMenuItem* SetHandler( const ChMsgHandler pHandler = 0 );
		ChMenuItem* SetHelpText( string strHelp );
		ChMenuItem* SetAccelerator( const chuint32 luChar, const chflag16 fsMods );
		chint16 GetIndex();

	protected:
		ChMenuItem( string strTitle, const ChMsgHandler pHandler, ChMenu *pParent );
