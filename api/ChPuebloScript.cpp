...
int ChPuebloScript::GetType( const string& strToken )
{
...
void ChPuebloScript::ProcessArguments( chint32 lStart, bool boolAttr )
{
	...
	while( true )
	{
		...
		else
		{
			...
			// update our style
			string 	strVal = TEXT( " " );		// updateattributes does not work
											 	//if strVal is empty
			AddNameValue( boolAttr, strAttr, strVal );
...
----------------------------------------------------------------------------*/
void ChPuebloScript::AddNameValue( bool boolAttr, string& strName, string& strValue )
{
	if ( strName.IsEmpty() )
	{
		return;
