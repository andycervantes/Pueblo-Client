...
ChHtmlViewObj::~ChHtmlViewObj() 
{
	if (m_boolDestroy)
	{
		...
		{
			string strName( m_pHtmlView->GetFrameName() );
...
string ChHtmlWnd::GetAnchorTarget( const char* pstrWindowName )
{
	if (pstrWindowName)
	{
...
string ChHtmlWnd::GetDocURL( const char* pstrWindowName )
{
	if (pstrWindowName)
	{
		ChHtmlView*		pView = GetHtmlViewByName( pstrWindowName );
...
string ChHtmlWnd::GetDocBaseURL( const char* pstrWindowName )
{
	if (pstrWindowName)
	{
		ChHtmlView*		pView = GetHtmlViewByName( pstrWindowName );
...
string ChHtmlWnd::GetDocTitle( const char* pstrWindowName )
{
	if (pstrWindowName)
	{
		ChHtmlView*		pView = GetHtmlViewByName( pstrWindowName );
...
void ChHtmlWnd::SetAnchorTarget( const string& strTarget,
									const char* pstrWindowName )
{
	if (pstrWindowName)
	{
...
ChHtmlView* ChHtmlWnd::GetHtmlViewByName( const string& strName )
{
												...
												CWnd* pFrame )
{
						...
						m_htmlViewList.GetCount() ))
	{
		...
		if ( boolAdd )
		{
			...
			if ( m_pPaneMgr )
			{
			 	pFrameObj = new ChHtmlViewObj( string( pstrName ), pView, (ChPaneMgr*)m_pPaneMgr );
			}
			else
			{
			 	pFrameObj = new ChHtmlViewObj( string( pstrName ), pView, (CFrameWnd*)pFrame );
			}
			ASSERT( pFrameObj );
