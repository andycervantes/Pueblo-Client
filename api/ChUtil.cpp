...
{
	public :
		ChDllLoadInfo( const string& strModule,  HMODULE hLibrary ) 	:
							 m_strModule( strModule ),
							 m_hLibrary( hLibrary )
							 {
							 }
 		const string& GetModuleName()		{ return m_strModule; }
...
string				ChUtil::m_strAppDirectory;
...
string			ChUtil::m_strPuebloDirectory;
...
bool ChUtil::Load( chparam resources, chint32 lKey, string& strData )
{
	bool	boolLoaded;
	...
	#if defined( CH_MSW )
	{
		...
		boolLoaded = strData.LoadString( (int)lKey );
...
bool ChUtil::GetTempFileName( string& strTmpFile, const char* pstrTempPath, 
...
void ChUtil::AddFileToTempList( const string& strFile )
{
...
bool ChUtil::ReadDirectory( void * pDirHandle, pChFileAttrs pAttrs, chuint flRequest )
{
	...
	if (!pfData->boolFirst)
	{
		if ( pfData->hFindFirst )
		{
			boolSuccess = FindNextFile(pfData->hFindFirst, &pfData->findFile);
		}
	...
	bool		boolSuccess = true;
		...
		{  
			if ( pDirHandle )
			{
				...
				if ( pDir->hFindFirst )
				{
					boolSuccess = FindClose( pDir->hFindFirst );
				}
				else
				{
					boolSuccess = true;
...
void ChUtil::GetAppDirectory( string& strDir )
{
	if (!m_directorySet)
...
bool ChUtil::CreateDirectoryTree( const string& strPath )
{
...
void ChUtil::EncryptString( string& strSource, bool boolEncrypt,
							char cDelimiter )
{
...
void ChUtil::EncodeAttributeString( const string& strSource,
									string& strDest )
{
	int		iIndex;
...
void ChUtil::DoEncrypt( string& strSource, char cDelimiter )
{
...
void ChUtil::DoDecrypt( string& strSource )
{
	string			strResult;
...
void ChUtil::AppendChar( string& strDest, const char cAppend,
							bool boolForceHex )
{
...
bool ChUtil::IsOldEncryptString( string& strSource )
{
	const char*		pstrSource = strSource;
...
void ChUtil::OldEncryptString( string& strSource, bool boolEncrypt,
								char cDelimiter )
{
	const string	strKey = "Spaulding Grey is the official parrot of "
...
int	 ChUtil::LoadString( UINT nID, string& strBuffer )
{
...
void ChUtil::HtmlAddNameValuePair( string& strData, const string& strName, const string& strValue )
{
	// Format the value
	string strTmp;
...
void ChUtil::HtmlAddNameValuePair( string& strData, const string& strName, int iValue )
{
	string strValue;
...
ChTmpFileManager::~ChTmpFileManager()		
{
	...
	while( pos )
	{
		string *pstrTmpFile = (string*)m_tmpFileList.GetNext( pos );
...
void ChUtil::AddModuleToLoadList( const string& strModule )
{
	bool boolAdd = true;
