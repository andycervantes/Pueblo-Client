...
// $Header: /home/cvs/chaco/api/ChHtmlView.cpp,v 2.28 1996/09/14 18:06:36 pritham Exp $
...
class ZapInlineData : public ChVisitor2<string, pChInlineImageData >  
{
	public:
	 	bool Visit( const string& key,  const pChInlineImageData& pInline )
		{
			delete pInline;
			return true;
		}
...
bool ChHtmlView::DisplayFile( const char* pstrFile,	const char* pstrMimeType /* = 0 */,
						chuint32 flOptions, const string& strURL )
{
...
bool ChHtmlView::DisplayImage( ChDib *pDib,	chuint32 flOptions,	const string& strURL )
{
	if ( flOptions & ChHtmlWnd::fileReplace )
	{
		NewPage();
...
----------------------------------------------------------------------------*/
bool ChHtmlView::PrefetchedLink( string &strLink )
{
	// if this append  had any prefetch tags, call the virtual function
	if (m_prefetchList.GetCount())
	{
...
void ChHtmlView::LoadBkPattern( const string& strURL )
{
...
void ChHtmlView::LoadInlineImage( const string& strURL, ChObjInline *pInLine )
{
...
void ChHtmlView::SpawnPlugInRequest( const string& strURL, ChPlugInStream* pStream,
...
void ChHtmlView::NotifySubmitForm( const string& strAction, 
						const string& strMD5, const string& strData )
{
