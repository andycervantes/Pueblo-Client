...
// $Header: /home/cvs/chaco/api/ChHtmMsg.cpp,v 2.53 1996/09/12 18:32:59 pritham Exp $
...
void ChHtmlView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	{
		m_hCursor = m_hHTMLCursors[ cursorHotspot ];
		::SetCursor( m_hHTMLCursors[ cursorHotspot ] );
	...
	else
	{
		...
		{
			m_hCursor = m_hHTMLCursors[ cursorNormal ];
		}
		else if ( iMouseOn == ChTxtWnd::locText )
		{
			m_hCursor = m_hHTMLCursors[ cursorText ];
		}
		else
		{
			m_hCursor = NULL;
...
void ChHtmlView::OnPrefetch( ChHTMLPrefetch* pFetch, const string& strDocURL )
{
...
void ChHtmlView::UpdateFontChange( const string& strOldProportional, int iOldProportionalSize,
						const string& strOldFixed, int iOldFixedSize )
{
	string		strNewFixed =  GetSettings()->GetFixedFontName();
	string		strNewProportional = GetSettings()->GetProportionalFontName();
