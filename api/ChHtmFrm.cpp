...
void ChHTMLForm::EscapeSpecialChars( string& strData )
{
	string strTmp;
	int i = 0;
...
void ChHTMLForm::AddNameValuePair( const char* pstrName, string& strVlaue )
{
...
void ChHTMLForm::SubmitForm()
{
	...
	switch ( GetMethod() )
	{
		...
		case methodXCHCMD:
		{
			string strAction( m_pstrAction ? m_pstrAction : TEXT( "" ) );
...
void ChResetBtn::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if ( nChar == VK_TAB )
	{
		...
		GetForm()->SetNextTabFocus( this, GetKeyState( VK_SHIFT ) & 0x8000 );
	}
