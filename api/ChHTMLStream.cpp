...
bool ChHtmlWnd::VisitedURL( const string &strLink, const char * pstrDefault /* = 0 */ )
{
...
void ChHtmlWnd::LoadURL( const string& strURL, const char* pstrWindowName /* = 0 */ )
{
	string strTarget( pstrWindowName ? pstrWindowName : TOP_WINDOW );
...
void ChHtmlWnd::PostURL( const string& strURL, const string& strData, 
...
void ChHtmlWnd::PrefetchURL( const string& strURL, const char * pstrDef /* = 0 */ )
{
...
void ChHtmlWnd::LoadURL( const string& strURL, ChHtmlReqInfo* pReqInfo )
{
...
void ChHtmlWnd::PostURL( const string& strURL, const string& strData, ChHtmlReqInfo* pReqInfo )
{
	ChHtmlView *pView = GetHtmlViewByName( pReqInfo->GetViewName() );
	ASSERT( pView );
...
ChDib* ChHtmlWnd::LoadDIB( const string& strMimeType, const char* pstrFilename )
{
		...
		case  ChHtmlReqInfo::loadInline :
		{
			...
			if ( pView && pView->GetPageNumber()  == pLoadInfo->GetViewID() )
			{
				...
				if ( pDib )
				{
					...
					pView->UpdateObject( pImage, sizeOld != sizeNew );
