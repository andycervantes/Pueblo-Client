...
// $Header: /home/cvs/chaco/api/ChPane.cpp,v 2.53 1996/09/12 18:33:04 pritham Exp $
...
const string 		ChPaneMgr::strTextName = "text";
const string 		ChPaneMgr::strGraphicName = "graphic";
...
ChPane* ChPaneMgr::FindPane( const string& strName )
{
	ChPane*		pPane = 0;
...
ChPane* ChPaneMgr::CreatePane( const string& strName, chparam userData,
...
void ChPaneMgr::DestroyPane( const string& strName )
{
	ChPosition	pos = m_list.GetHeadPosition();
...
string ChPaneMgr::FindClassName( bool boolCloseable, chuint16 suIconID )
{
...
string ChPaneMgr::RegisterPaneClass( bool boolCloseable, chuint16 suIconID )
{
	#if defined( CH_MSW )
	{
		...
		boolClassCreated = AfxRegisterClass( &classStruct );
		if (!boolClassCreated)
...
ChPane* ChPane::SetTitle( const string& strTitle )
{
...
bool ChPane::GetPaneNameAttr( const char* pstrArgs, string& strPaneName )
{
	bool	boolFound;
...
bool ChPane::GetPaneAttrs( const char* pstrArgs, ChPaneCmd& paneCommand,
							string& strPaneName, string& strPaneTitle,
							chflag32& flOptions )
{
	bool	boolFound;
	string	strPaneCommand;
...
ChPane::ChPane( const string& strName, ChPaneMgr* pPaneMgr,
...
ChPane::ChPane( PaneType type, const string& strName, ChPaneMgr* pPaneMgr ) :
		m_type( type ),
		m_pPaneMgr( pPaneMgr ),
		m_strName( strName ),
		m_userData( 0 ),
...
ChPaneWindowClass::ChPaneWindowClass( const string& strName,
										bool boolCloseable,
										chuint16 suIconID ) :
				m_strName( strName ),
...
ChPaneWindowClass::~ChPaneWindowClass()
{
}
