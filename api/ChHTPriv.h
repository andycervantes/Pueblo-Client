...
// $Header: /home/cvs/chaco/api/ChHTPriv.h,v 2.40 1996/04/23 21:36:36 pritham Exp $
...
class ChThreadStringBlock
{
	public:
		...
		bool Check( const string& strName ) { return false; }
		void Release() {}
};
...
typedef ChSplay<string, int>	ChHTTPVisitedList;            
typedef ChSplay<string, sockaddr_in>	ChSocketAddrList;            
#else 
...
class ChHTTPConnStream  : public ChHTTPStream
{
	public :
	    ...
	    virtual const string& GetContentEncoding();
	    virtual const string& GetLastModified();
	    virtual const string& GetTargetWindowName();
...
{
	public:
		ChHTTPInfo( ChHTTPSocketConn* pHTTPConn,  const string& strURL,
		...
		inline const string& GetLocalName()
							{
		...
		const string&	GetFileName()		{ return m_strLocalName; }
		chint32			GetDataLength()		{ return m_luBodyLen; }
		const string&   GetTargetWindowName(){ return m_strTargetWndName; }
		const string&   GetLastModified()	{ return m_strLastModified; }
		...
		bool			IsZipped();
		const string&	GetZipFileName()	{ return m_strZipName; }
		...
		void			SetPostData( const char *pstrData, chint32 lLen )		
								{
									...
									{
										string strTmp( pstrData, (int)lLen );
		...
		static bool 	CreateAndValidateCacheDir( string& strDir );
		...
		static void 			AddToSocketAddrList( const string& strHost, const sockaddr_in& addrIn );
		static bool 			FindSocketAddr( const string& strHost, sockaddr_in& addrIn );
	...
	private :
		...
		bool 	GetNextHdr( char* &pstrHdr, char* &pstrData, char* &pstrNext, int iDelimType );
		void 	ProcessBody();
...
CH_GLOBAL_FUNC( void )
UpdateWaitRequest( ChHTTPInfo *pHTTPInfo, const string strNewURL );
