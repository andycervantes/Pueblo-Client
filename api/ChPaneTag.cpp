	...
	for (int i = 0; i < iArgCount; i++)
	{
		switch ( pList[i].iArgType )
		{
			...
			{								/* Target names are case-
				...
				string		strName( (const char*)pList[i].argVal );
			...
			case ARG_ACTION:
			{
				...
				string		strAction( (const char*)pList[i].argVal );
			...
			{
				chuint32	luOption = 0;
				string		strOptions( (const char*)pList[i].argVal );
			...
			{
				chuint32	iScroll = 0;
				string		strScrolling( (const char*)pList[i].argVal );
