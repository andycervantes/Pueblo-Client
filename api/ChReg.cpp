...
// $Header: /home/cvs/chaco/api/ChReg.cpp,v 2.13 1996/07/18 20:21:32 pritham Exp $
...
chint32 RegQueryValue( HKEY hKey, const string& strLabel, char* pstrBuffer,
					   chint32* pLen )
{
	cerr << "RegQueryValue( " << hKey << ", " << strLabel << " )" << endl;

	string strDbKey = (*(string*)hKey) + "/" + strLabel;
...
chint32 RegSetValue( HKEY hKey, const string& strLabel, chuint32 luFlags,
...
chint32 RegCreateKey( HKEY hKey, char *pstrSoftwareKey, HKEY* pNewKey )
{
	...
	if (hKey == HKEY_CLASSES_ROOT) {
		pStr = new string(pstrSoftwareKey);
	} else {
		pStr = new string(*(string*)hKey);
		*pStr += "/";
...
ChRegistry::ChRegistry( const string& strCompanyName,
						const string& strProduct, const string& strGroup,
...
ChRegistry::ChRegistry( const string& strGroup, const string& strVersion ) :
...
bool ChRegistry::GetHelpPath( string& strHelpFiles )
{
...
bool ChRegistry::Erase( const string& strLabel )
{
	chint32		lRetVal;
...
bool ChRegistry::WriteBool( const string& strLabel, bool boolVal )
{
#ifdef CH_MSW
	string	strVal( boolVal ? strTrue : strFalse );
...
bool ChRegistry::Write( const string& strLabel, chint32 lVal )
{
	chint32	lRetVal;
...
bool ChRegistry::Write( const string& strLabel, const string& strVal )
{
	chint32	lRetVal;
...
bool ChRegistry::Read( const string& strLabel, chint32& lVal,
							chint32 lDefault )
{
	chint32	lRetVal;
...
bool ChRegistry::Read( const string& strLabel, chint16& sVal,
							chint16 sDefault )
{
	bool		boolSuccess;
	chint32		lVal;
...
bool ChRegistry::ReadBool( const string& strLabel, bool& boolVal,
							bool boolDefault )
{
	bool		boolSuccess;
	string*		pstrDefault;
	...
	string stringTrue(strTrue);
	string stringFalse(strFalse);
...
bool ChRegistry::Read( const string& strLabel, string& strVal,
...
void ChRegistry::Construct( const string& strCompanyName,
							const string& strProduct, const string& strGroup,
							const string& strVersion )
{
	#if defined( CH_MSW )
