...
class  ChHtmlParser
{
	public :
		...
		ChHtmlTag* 	GetHTMLTagInstance( const string& strToken );
		...
		void ClearUserArgs();
		void UpdateUserArgs( const string& strName, const string& strValue );
		...
		void LoadInline( const char* pstrURL, ChObjInline *pImage )
				{
					m_pHtmlView->LoadInlineImage( string(pstrURL), pImage );
				}
		...
		void GetObjectAttrs( bool& boolObj, chuint32& luAttr, ChSize& sizeObj, 
	...
	private : // core Parser methods
		...
		void Initialize();
		ChHtmlTag* CreateInstanceFromName( const string& strName );
