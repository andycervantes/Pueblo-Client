...
// $Header: /home/cvs/chaco/api/ChSock.cpp,v 2.19 1996/09/15 19:13:41 coyote Exp $
...
string 			ChChacoSocket::m_strSocksHostName;
...
void sockinetaddr::SetAddr( const char *pstrHostName )
{
	...
	if ( (m_sockAddrIn.sin_addr.s_addr = inet_addr( pstrHostName )) == -1)
	{
...
void ChChacoSocket::GetErrorMsg( int iError, string& strOutput )
{
	switch( iError )
	{
		case 0:
							...
							LPCTSTR lpszSocketAddress )
{
	...
	if (GetSyncSocket())
	{
		boolSuccess = GetSyncSocket()->Create( nSocketPort, nSocketType,
												lpszSocketAddress );
	}
	else
	{
...
bool ChChacoSocket::Attach( SOCKET hSocket )
{
	...
	if (GetSyncSocket())
	{
		boolSuccess = GetSyncSocket()->Attach( hSocket );
	}
	else
	{
		if (boolSuccess = GetAsyncSocket()->Attach( hSocket ))
		{
											// Set to non-blocking
...
SOCKET ChChacoSocket::Detach()
{
	...
	if (GetSyncSocket())
	{
		boolSuccess = GetSyncSocket()->Detach();
	}
	else
	{
		boolSuccess = GetAsyncSocket()->Detach();
	}
	return boolSuccess;
}
...
bool ChChacoSocket::Connect( const SOCKADDR* lpSockAddr, int nSockAddrLen )
{
	...
	else
	{
		...
		boolSuccess = GetAsyncSocket()->Connect( lpSockAddr, nSockAddrLen );  
...
void ChChacoSocket::UpdateSocketPreferences( const string& strProduct )
{
	bool boolUseProxies;
										...
										chparam userData )
{
	...
	if ( (sa.m_sockAddrIn.sin_addr.s_addr = inet_addr( pstrHostName )) == -1)
	{
		HANDLE	hWaitOn = WSAAsyncGetHostByName( ChChacoSocket::GetWSAWnd(),
													CH_MSG_WSA_PACKET_RECEIVED,
													pstrHostName,
