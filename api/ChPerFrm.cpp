...
// $Header: /home/cvs/chaco/api/ChPerFrm.cpp,v 2.9 1996/09/12 18:33:06 pritham Exp $
...
const string	ChPersistentFrame::strProfileRect( "window rect" );
const string	ChPersistentFrame::strProfileIcon( "iconic" );
const string	ChPersistentFrame::strProfileMax( "maximized" );
const string	ChPersistentFrame::strProfileTool( "toolbar" );
const string	ChPersistentFrame::strProfileStatus( "status bar" );
#endif
...
void ChPersistentFrame::SetLabel( const string& strLabel )
{
	m_strLabel = strLabel;

	#if defined( CH_MSW ) && !defined( CH_PUEBLO_PLUGIN )
	{
		string		strTempLabel = GetLabel();
...
void ChPersistentFrame::OnDestroy()
{
	...
	boolSuccess = GetWindowPlacement( &wndpl );
...
void ChPersistentFrame::ActivateFrame( int nCmdShow )
{
	...
	if (boolFirstTime)
	{
		...
		boolSuccess = SetWindowPlacement( &wndpl );
	}
