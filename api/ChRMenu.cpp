...
// $Header: /home/cvs/chaco/api/ChRMenu.cpp,v 2.16 1996/03/08 10:08:43 pritham Exp $
...
ChRMenu::ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChNativeMenu * pNatMenu,
...
ChRMenu::ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChNativeMenu * pMenuBar  ):
					m_pMenuMgr( pMenuMgr ),
					m_pMyMenu(0),
					m_strTitle(strTitle)
{
...
ChRMenu::ChRMenu( ChRMenuMgr* pMenuMgr, const string& strTitle, ChRMenuItem *pItem  ):
					m_pMenuMgr( pMenuMgr ),
					m_pMyMenu(0),
					m_strTitle(strTitle)
{
...
----------------------------------------------------------------------------*/
ChMenu* ChRMenu::Install( ChMenu *pMenu,  const string& strModuleName, bool boolToHead)
{
	string	strBlockModuleName;
	chuint32	luCount = 0;
...
ChRMenuBlock* ChRMenu::FindBlock(string strModName, chuint32 *plBegin)
{
	ChRMenuBlock*	pBlock;
...
string& ChRMenuItem::GetHelpText()
{
 	ThrowIfEmptyMenu(&m_items);
...
string  ChRMenuItem::MakeDisplayTitle( )
{
	string strDispTitle = GetTitle();
	ChPosition pos = m_items.GetHeadPosition();
...
string ChMenuItem::GetDisplayTitle()
{
	string	strAccel = m_strTitle;
...
string  ChRMenuItem::SplitDisplayTitle(const string & strDisplayTitle )
{
#ifdef CH_MSW
	string strTitle = strDisplayTitle.SpanExcluding("\t");
	return strTitle;
#else
	NOT_IMPLEMENTED;
	return "";
...
ChMenu* ChRMenuMgr::Install( ChMenu *pMenu, const string& strModuleName, bool boolToHead )
{
...
{
	string strModuleName = GetCore()->GetModuleName(idModule);
...
ChRMenu *ChRMenuMgr::GetRMenu(string strTitle)
{
...
bool ChRMenuMgr::GetMessageString( UINT nID, string& rMessage )
{
...
ChRMenuBlock::ChRMenuBlock( string strModName, ChRMenu *pRMenu  ) :
...
{
	string	strTitle = pRItem->GetTitle();	  // need to save these for later
#ifdef CH_MSW
...
void ChRMenuBlock::AddAccelerator( ChMenuItemId id, const chuint32 luChar, const chflag16 fsMods )
{
	...
	if (m_hAccelTable)
	{
		...
		{
			if (pAccel[j].cmd == id)	// should we allow more than one per id?? only one would be on 
										// menu display.
			{
				delete [] pAccel;
				return;
...
string ChRMenuBlock::MakeSeparatorTitle( ChMenuItemId id ) const
{        
	#if 1
	//#if defined( CH_MSW ) && defined( CH_ARCH_16 )
